...
| Validation surface | pass | 60 test files; 7 workflow files. Both figures are derived from the tree by `tests/test_published_claims.py::test_stated_file_counts_match_the_tree`, which fails if any authored doc states a count the tree no longer supports — this row said 35 and 6 long after both had moved. |
